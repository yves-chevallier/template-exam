...
\newcommand{\texsmithQuestion}[2][]{%
  \texsmithCloseParts
  \def\texsmithTmp{#2}%
  \def\texsmithPts{#1}%
  \if\relax\detokenize{\texsmithTmp}\relax
    \if\relax\detokenize{\texsmithPts}\relax
      \question
    \else
      \question[\texsmithPts]%
    \fi
  \else
    \if\relax\detokenize{\texsmithPts}\relax
      \titledquestion{#2}%
    \else
      \titledquestion{#2}[\texsmithPts]%
    \fi
  \fi
}

\newcommand{\texsmithPart}[2][]{%
  \texsmithCloseSubparts
...
\newcommand{\texsmithSubpart}[2][]{%
  ...
  \else
    \if\relax\detokenize{\texsmithPts}\relax
      \subpart #2%
    \else
      \subpart[\texsmithPts] #2%
    \fi
  \fi
}

\newcommand{\texsmithSubsubpart}[2][]{%
  \iftexsmithPartsOpen\else
    \begin{parts}%
    \texsmithPartsOpentrue
  \fi
  ...
  \if\relax\detokenize{\texsmithTmp}\relax
    \if\relax\detokenize{\texsmithPts}\relax
      \subsubpart
    \else
      \subsubpart[\texsmithPts]%
    \fi
  \else
    \if\relax\detokenize{\texsmithPts}\relax
      \subsubpart #2%
    \else
      \subsubpart[\texsmithPts] #2%
    \fi
  \fi
}
